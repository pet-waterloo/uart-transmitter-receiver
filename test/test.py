...
# =============================================================
# Helper (Receiver Exhaustive Tests)
# =============================================================

def codeword_str_to_int(codeword_str: str) -> int:
    """Convert codeword string in order [c0 c1 d0 c2 d1 d2 d3] into integer whose bit i = char i."""
    val = 0
    for i, ch in enumerate(codeword_str):
        if ch == '1':
            val |= (1 << i)
    return val

async def uart_send_codeword_bits(dut, code_int: int, cycles_per_bit: int):
    """Send one framed UART word carrying a 7-bit Hamming code (start + 7 data bits + stop)."""
    # Idle (1 bit time)
    await send_idle_bits(dut, dut.ui_in, cycles_per_bit)
    # Start
    await send_start_bit(dut, dut.ui_in, cycles_per_bit)
    # Data bits LSB first
    await send_data_bits(dut, dut.ui_in, f"{code_int:07b}"[::-1], cycles_per_bit)
    # Stop
    await send_stop_bit(dut, dut.ui_in, cycles_per_bit)
    # Idle guard
    await send_idle_bits(dut, dut.ui_in, cycles_per_bit)

def sample_decoded_outputs(dut):
    d0 = (dut.uo_out.value >> 2) & 0x1
    d1 = (dut.uo_out.value >> 3) & 0x1
    d2 = (dut.uo_out.value >> 5) & 0x1
    d3 = (dut.uo_out.value >> 6) & 0x1
    decode_out = (d3 << 3) | (d2 << 2) | (d1 << 1) | d0
    syndrome_out = dut.uio_out.value & 0x7
    valid_out = (dut.uo_out.value >> 7) & 0x1
    return decode_out, syndrome_out, valid_out

# =============================================================
# Receiver Exhaustive Tests
# =============================================================

@cocotb.test()
async def test_receiver_all_valid_codewords(dut):
    """Exhaustively send all valid Hamming(7,4) codewords and verify decode & syndrome."""
    dut._log.info("Starting exhaustive valid codeword receiver test")
    clock = Clock(dut.clk, 50, units="us")
    cocotb.start_soon(clock.start())
    cycles_per_bit = BAUD_CYCLES
    await reset_dut(dut)

    for data_bits_str, codeword_str in HAMMING_CODE_TABLE.items():
        await reset_dut(dut)
        data_expected = int(data_bits_str, 2)
        code_int = codeword_str_to_int(codeword_str)

        dut._log.info(f"Sending codeword (data={data_bits_str} -> code={codeword_str})")
        await uart_send_codeword_bits(dut, code_int, cycles_per_bit)

        # Allow a few cycles for decoder enable + output
        await ClockCycles(dut.clk, 4)

        decode_out, syndrome_out, valid_out = sample_decoded_outputs(dut)
        dut._log.info(f"RESULT data={decode_out:04b} syndrome={syndrome_out:03b} valid={valid_out}")

        if syndrome_out != 0:
            dut._log.error(f"SYNDROME ERROR: expected 000 got {syndrome_out:03b} (data={data_bits_str})")
        if decode_out != data_expected:
            dut._log.error(f"DECODE ERROR: expected {data_expected:04b} got {decode_out:04b} (data={data_bits_str})")
        if valid_out != 1:
            dut._log.error(f"VALID ERROR: expected 1 got {valid_out} (data={data_bits_str})")

        assert syndrome_out == 0, f"Syndrome not zero for valid code {codeword_str}"
        assert decode_out == data_expected, f"Decoded mismatch for data {data_bits_str}"
        assert valid_out == 1, "Valid flag not asserted"

    dut._log.info("All valid codewords decoded correctly")

@cocotb.test()
async def test_receiver_all_single_bit_errors(dut):
    """Send every valid codeword with each possible single-bit error injected."""
    dut._log.info("Starting exhaustive single-bit error receiver test")
    clock = Clock(dut.clk, 50, units="us")
    cocotb.start_soon(clock.start())
    cycles_per_bit = BAUD_CYCLES
    await reset_dut(dut)
    ...
    for data_bits_str, codeword_str in HAMMING_CODE_TABLE.items():
        data_expected = int(data_bits_str, 2)
        original_code_int = codeword_str_to_int(codeword_str)

        for bit_idx in range(7):
            await reset_dut(dut)
            errored_code_int = original_code_int ^ (1 << bit_idx)
            dut._log.info(f"Data={data_bits_str} code={codeword_str} flip_bit={bit_idx} -> send={errored_code_int:07b}")

            await uart_send_codeword_bits(dut, errored_code_int, cycles_per_bit)
            await ClockCycles(dut.clk, 4)

            decode_out, syndrome_out, valid_out = sample_decoded_outputs(dut)
            dut._log.info(f"RESULT flip={bit_idx} decode={decode_out:04b} syndrome={syndrome_out:03b} valid={valid_out}")

            if syndrome_out == 0:
                dut._log.error(f"SYNDROME ERROR: expected non-zero, got 000 (flip={bit_idx}, data={data_bits_str})")
            if decode_out != data_expected:
                dut._log.error(f"DECODE ERROR: expected {data_expected:04b} got {decode_out:04b} (flip={bit_idx}, data={data_bits_str})")
            if valid_out != 1:
                dut._log.error(f"VALID ERROR: expected 1 got {valid_out} (flip={bit_idx}, data={data_bits_str})")

            assert syndrome_out != 0, f"No syndrome for single-bit error (bit {bit_idx})"
            assert decode_out == data_expected, f"Decoded mismatch after correction (bit {bit_idx})"
            assert valid_out == 1, "Valid flag not asserted after correction"

    dut._log.info("All single-bit error cases corrected successfully")
